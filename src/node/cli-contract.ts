...
export const COMMANDS: Record<string, Command> = {
  ...
  today: read("", "Entry[]", `${roll} include-archive ${paging}`),
  recent: read("", "Entry[]", `${roll} include-archive ${paging}`),
  ...
  history: read("[file]", "{commit, author, date, subject, patch}[], or {id, title, path, deletedAt, commit}[] for the Roll's removals", roll, 1),
  restore: write("<entry> [commit]", "{entry, from, unchanged, restored} — an earlier version, or a deleted entry put back", roll, 2),
